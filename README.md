...
- [Complete Unraid installation guide](docs/unraid-install.md)
- [Setup, template, authentication, network, and generated-file options](docs/options.md)
- [Manual deployment smoke test](tests/smoke.md)
...
The script prompts for secrets without echoing them. It generates a unique, persistent account ID, a local CA and IP certificate, a GPG encryption key, and readable YAML configuration. It is safe to rerun: existing identity material and configuration are preserved. `--force-config` explicitly backs up and regenerates YAML, but never rotates identity keys. Users with DNS can additionally pass `--hostname omni.home.arpa`; otherwise every advertised endpoint uses the IP.
...
Import `/mnt/user/appdata/omni/tls/ca.crt` into the trust store of every browser/administrative workstation that connects to Omni. The certificate covers the LAN IP and loopback address, plus the optional hostname when supplied.
...
3. Open `https://192.168.1.10:8443`, accept the EULA, and sign in.

For Authentik or generic OIDC, install only `Omni`.

For descriptions of every setup flag and Unraid template field, see the [options reference](docs/options.md).
...
2. Set the strict redirect URI to `https://192.168.1.10:8443/oidc/consume`, substituting your Unraid IP (or optional hostname).
...
Run `./tests/test.sh`. It validates template XML and exercises all setup modes without creating real GPG keys or pulling images. Docker smoke checks are documented in `tests/smoke.md` because they require host networking, `/dev/net/tun`, and a real deployment network.
